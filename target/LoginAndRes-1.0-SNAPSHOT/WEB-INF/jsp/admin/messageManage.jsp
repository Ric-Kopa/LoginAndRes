    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .modal-content textarea {
            ...
            border: 2px solid #e1e1e1; border-radius: 5px; resize: vertical;
        }
...
<body>
                ...
                <tbody>
                    <c:forEach var="msg" items="${messages}">
                        <tr>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${not empty msg.avatar}">
                                        <img src="${pageContext.request.contextPath}/${msg.avatar}" class="avatar" alt="头像">
                                    </c:when>
                                    <c:otherwise>
                                        <div style="width:40px;height:40px;border-radius:50%;background:#667eea;color:white;display:flex;align-items:center;justify-content:center;font-size:18px;">${msg.nickname.charAt(0)}</div>
                                    </c:otherwise>
                                </c:choose>
                                ${msg.nickname}
                            </td>
                            ...
                            <td>
                                <button class="btn-edit" onclick="showEditModal(${msg.id}, '${msg.content}')">编辑</button>
    ...
    <script>
        ...
        function deleteMessage(id) {
            if (confirm('确定删除该留言？')) {
                location.href = '${pageContext.request.contextPath}/admin?action=deleteMessage&id=' + id;
            }
        }
    </script>
</body>
